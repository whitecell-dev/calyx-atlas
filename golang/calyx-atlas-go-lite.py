...
class CalyxGoNexusAnalyzer:
    ...
    def extract_exported_symbols(self, functions: List[str], interfaces: List[str]) -> List[str]:
        ...
        exported = [s for s in functions if s.split('.')[-1].startswith(r'[A-Z]')]
        exported.extend([i for i in interfaces if i.startswith(r'[A-Z]')])
    ...
    def detect_layer(self, pkg_name: str) -> str:
        ...
        name = pkg_name.lower()
        if name in ["core", "server", "main", "service"]: return "NEXUS"
        if name in ["config", "api", "client", "io"]: return "BRIDGE"
        if name in ["math", "util", "helpers", "testutil"]: return "ALBEO" # Pure/Utility
        if name in ["store", "repository", "db"]: return "MNEME" # Memory/State
        return "UTILITY"
    ...
    def analyze_go_file(self, path: Path) -> Optional[CalyxGoFile]:
        ...
        init_funcs = len([f for f in functions if f == 'init'])
        ...
        for pkg_name, files in self.packages.items():
            ...
            for file_name in files:
                ...
                for imp in file_obj.imports:
                    # Heuristic: Check if import path contains a recognized package name
                    if any(p in imp for p in self.packages if p != pkg_name):
                        pkg_deps.add(imp)
